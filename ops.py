...
class ConnectorRun(bpy.types.Operator):
    bl_idname = "sdn.connector_run"
    bl_label = "Run"
    bl_translation_context = ctxt
    ...
    def poll(cls, context):
        return bpy.context.scene.sdn.comfyui_tree in bpy.data.node_groups

    def execute(self, context):
        tree_name = bpy.context.scene.sdn.comfyui_tree
        tree: CFNodeTree = bpy.data.node_groups.get(tree_name)
        if not tree:
            return {"CANCELLED"}
        tree.execute()
        return {"FINISHED"}
